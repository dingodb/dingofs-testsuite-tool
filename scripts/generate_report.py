...
def generate_text_summary(tool, output_dir, data, scenario, mount):
    ...
    summary.append("")
    summary.append(f"| Parameter | Value |")
    summary.append(f"|-----------|-------|")
    ...
    summary.append(f"- This Summary: `{output_dir}/summary.md`")
    summary.append("")
    summary.append("---")
    summary.append("*Report generated by DingoFS Storage Benchmark Tools*")
...
def generate_fio_markdown_metrics(summary, data):
    ...
    for job in data["jobs"]:
        ...
        if read_data:
            ...
            summary.append(f"| Latency (mean) | {lat_mean} |")
            summary.append(f"| Latency (min) | {lat_min} |")
            summary.append(f"| Latency (max) | {lat_max} |")
            summary.append("")

        if write_data:
            summary.append("| Metric | Write |")
            summary.append("|--------|-------|")
...
def generate_vdbench_markdown_metrics(summary, data):
    ...
    for key in ["throughput", "IOPS", "response_time"]:
        ...
        if avg_key in metrics:
            ...
            summary.append(f"| {key.capitalize()} | {avg} | {min_val} | {max_val} |")
...
def generate_fio_summary_tables_text(direct_0, direct_1, rw_type):
    ...
    # Determine column header based on rw_type
    if rw_type in ("seq_read", "rand_read"):
        bw_label = "READ_BANDWIDTH (MiB/s)"
    else:
        bw_label = "WRITE_BANDWIDTH (MiB/s)"
    ...
    lines.append("")
    lines.append(f"| Block Size | Num Jobs | {bw_label} | Latency (us) |")
    lines.append(f"|-------------|----------|------------------|---------------|")
